...
export default function HeaderOption({ children, name, to, ...props }) {
    ...
    return (
        ...
        >
            <Link
                to={to}
                className={`flex gap-[2px] text-md font-bold mb-1 uppercase transition-all hover:opacity-100 tracking-tighter text-nowrap ${
                    isLocation() ? "opacity-100" : "opacity-80"
                }`}
            >
                {name}
                {children && (
                    <div className={`relative w-[0.7rem]`}>
            ...
            {children && (
                <div
                    className={`
                        landing-header-option-component
